...
new_login = '''        String savedUsername = rememberedUsername(company);
        ...
        Button login = button("Войти");
        login.setOnClickListener(v -> doLogin(username.getText().toString(), password.getText().toString(), remember.isChecked()));
        box.addView(login, new LinearLayout.LayoutParams(-1, dp(54)));
        replaceBody(box);
    }
...
new_login_success = '''                V4Core.SecureStore.token(this, targetCompany, token);
                if (rememberCredentials) saveRememberedCredentials(targetCompany, username.trim(), password);
                else clearRememberedCredentials(targetCompany);
                getSharedPreferences("boni_v4_plain", MODE_PRIVATE).edit()
                        .putString("selected_company", targetCompany).apply();
'''
...
new_download = '''    private boolean attachmentFallbackAllowed(Exception e) {
    ...
    private String bestMimeType(String serverMime, String name) {
        ...
        if (dot >= 0 && dot + 1 < safe.length()) {
            String guessed = MimeTypeMap.getSingleton().getMimeTypeFromExtension(safe.substring(dot + 1).toLowerCase());
    ...
    private void openDownloadedFile(V4Core.DownloadResult r, String name) {
        ...
        try {
            ...
            try { startActivity(Intent.createChooser(generic, "Открыть вложение")); }
            catch (ActivityNotFoundException second) {
                Toast.makeText(this, "Файл скачан в приватное хранилище, но на телефоне нет приложения для его открытия", Toast.LENGTH_LONG).show();
            }
        }
    }
    ...
    private void downloadAndOpen(String primaryPath, String fallbackPath, String name) {
        ...
        io.execute(() -> {
            ...
            try {
                ...
                try { r = V4Core.Api.download(this, c, token, primaryPath, name); }
                catch (Exception e) {
                    firstError = e;
                    if (fallbackPath == null || fallbackPath.isBlank() || fallbackPath.equals(primaryPath) || !attachmentFallbackAllowed(e)) throw e;
                ...
                runOnUiThread(() -> {
                    ...
                    if (!finalUsedPath.equals(primaryPath)) Toast.makeText(this, "Вложение открыто через совместимый режим", Toast.LENGTH_SHORT).show();
                });
            } catch (Exception e) {
                ...
                runOnUiThread(() -> {
                    ...
                    if (first instanceof V4Core.ApiException) detail.append("\\nПервый путь: HTTP ").append(((V4Core.ApiException) first).status);
                    if (e instanceof V4Core.ApiException) detail.append("\\nПоследний путь: HTTP ").append(((V4Core.ApiException) e).status);
                    detail.append("\\n").append(failedPath);
...
for label, ok in {
    "encrypted password": 'V4Core.SecureStore.put(this, "remember_password_"' in s,
    "encrypted password read": 'V4Core.SecureStore.get(this, "remember_password_"' in s,
    "attachment file endpoint": '/api/mobile/v4/chat/attachment/file/' in s,
    "attachment message fallback": '/api/mobile/v4/chat/attachment/message/' in s,
    "MIME inference": 'MimeTypeMap.getSingleton().getMimeTypeFromExtension' in s,
}.items():
    if not ok: raise SystemExit(f"V4C generated security check failed: {label}")

MAIN.write_text(s, encoding="utf-8", newline="\n")

build = BUILD.read_text(encoding="utf-8").replace("\r\n", "\n")
build = replace_once(build, "versionCode 21", "versionCode 22", "versionCode 22")
build = replace_once(build, "versionName '4.0b-native-internal'", "versionName '4.0c-native-internal'", "versionName V4C")
BUILD.write_text(build, encoding="utf-8", newline="\n")

print("V4C_ATTACHMENT_CREDENTIAL_PATCH=OK")
print("V4C_VERSION_CODE=22")
print("V4C_VERSION_NAME=4.0c-native-internal")
